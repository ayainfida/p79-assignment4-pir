...
class LWEMethods:
    @staticmethod
    def sample_error_vector(N: int, q: int, B: int = 3) -> np.ndarray[RingElement]:
        """
        ...
        """
        error = np.random.randint(-B, B + 1, size=N)

        return RingElement.get_ring_vector(error, q)

    @staticmethod
    def sample_secret_vector(N: int, q: int) -> np.ndarray[RingElement]:
        """
        ...
        """
        secret = np.random.randint(-q, q, size=N)
        return RingElement.get_ring_vector(secret, q)
    
    @staticmethod
    def generate_matrix_A(N: int, n: int, q: int, seed: int) -> np.ndarray[RingElement]:
        """
        ...
        """
        np.random.seed(seed)
        A = np.random.randint(-q, q, size=(N, n))
        return RingElement.get_ring_vector(A.flatten(), q).reshape(N, n)
    


if __name__ == "__main__":
    # Example usage of the LWE methods
    error_vector = LWEMethods.sample_error_vector(N=10, q =32)
